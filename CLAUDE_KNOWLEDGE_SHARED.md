...
- **Key grouped elements**: `<configdb>`, `<user type="admin|monitor">`, `<monitoring>`, `<authentication>`, `<ldap>`, `<jira>`, `<environment_types>`, `<redis>`, `<timeouts>`, `<features>`, `<testing>`, `<dbtype>`
...
- **Nullable INT column for per-host overrides**: NULL means "use the site-wide default", not "no timeout". Column COMMENTs should explain WHY null is meaningful (e.g., "set per-host for WAN or slow targets"), not just what null means
...
- **`@` error suppressor leaves dirty error handlers**: PHPUnit 11 detects handler
  stack changes and marks tests as "risky". Use explicit guards instead of `@`
  (e.g. `if (session_status() !== PHP_SESSION_ACTIVE)` makes `@session_start()`
  unnecessary).
- **`@codeCoverageIgnore` — use per-line, NOT Start/End blocks inside methods**:
  Start/End blocks inside a method can leak and swallow subsequent methods. Use
  `// @codeCoverageIgnore` on specific untestable lines instead.
- **Controller DDL coverage pattern**:
  - `createTable()` with `IF NOT EXISTS` is safe to call in integration tests —
    covers `doDDL()` success path in ControllerBase.
  - `dropTable()` method bodies get `// @codeCoverageIgnore` — destructive, not
    safe to test against a live DB.
  - DB-failure throw lines in `get()`, `getSome()`, `add()`, `delete()`,
    `update()` get `// @codeCoverageIgnore` — require DB malfunction.
- **Pure-function extraction for testability** (see also Config, ConfigUpgrader):
  When a method reads live state (config file, DB, session), extract the pure
  transformation logic into a public static method that takes inputs and returns
  outputs. Tests call the static method directly. Example: `LDAP::authenticateLocally()`
  extracted from `authenticate()` so local-auth logic is testable without a live
  config that may have LDAP enabled.

### Controller Layer Bug History
Bugs found by TDD that had been silently broken in production:
- **`ControllerBase::__construct` catch**: used `$this->dbh->error` — null
  dereference when `DBConnection` throws (dbh never assigned). Fixed: `$e->getMessage()`.
- **`HostGroupController` and `HostGroupMapController` `createTable()` missing
  `IF NOT EXISTS`**: re-running `deployDDL.php` after setup would fail with
  "table already exists". `HostController` had it; the others didn't.
- All three concrete controllers used `$this->_dbh` (underscore) but
  ControllerBase declares `$this->dbh` (no underscore) — every method failed.
- Full history of 15+ controller bugs in the local CLAUDE.md.
